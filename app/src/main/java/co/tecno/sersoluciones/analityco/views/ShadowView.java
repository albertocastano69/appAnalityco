...
public class ShadowView extends View {
    ...
    private void initialize(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        setBackground(makeCubicGradientScrimDrawable(8, gravity));
    }

    /**
     * Creates an approximated cubic gradient using a multi-stop linear gradient.
     */
    private Drawable makeCubicGradientScrimDrawable(int numStops, int gravity) {

        // Generate a cache key by hashing together the inputs, based on the method described in the Effective Java book
        int cacheKeyHash = 0x44000000;
        ...
        int red = Color.red(0x44000000);
        int green = Color.green(0x44000000);
        int blue = Color.blue(0x44000000);
        int alpha = Color.alpha(0x44000000);

        for (int i = 0; i < numStops; i++) {
            float x = i * 1f / (numStops - 1);
            float opacity = constrain((float) Math.pow(x, 3));
    ...
    private float constrain(float v) {
        return Math.max((float) 0, Math.min((float) 1, v));
    }
