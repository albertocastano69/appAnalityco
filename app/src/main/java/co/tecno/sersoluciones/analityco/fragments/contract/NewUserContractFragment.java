 ...
 **/
public class NewUserContractFragment extends Fragment implements
    ...
    private UserProject userContract;
    private ArrayList<Roles> rolInfoList;
    private ArrayList<UserProject> listUsers;
    private Date finishDateContract;
    private String finishDate;

    public static NewUserContractFragment newInstanceActivityUser(String companId, String contractId, ArrayList<UserProject> userContractList, String finishdate) {
        int ContextFrom = 0;
        NewUserContractFragment fragment = new NewUserContractFragment();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, companId);
        args.putString(ARG_PARAM2, contractId);
        args.putInt(ARG_PARAM3, ContextFrom);
        args.putParcelableArrayList(ARG_PARAM5, userContractList);
        args.putString(ARG_PARAM6, finishdate);
        fragment.setArguments(args);
        return fragment;
    }

    public static NewUserContractFragment newInstanceActivityUserEdit(String companId, String contractId, UserProject userPro, String finishdate) {
        int ContextFrom = 1;
        NewUserContractFragment fragment = new NewUserContractFragment();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, companId);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
           /* mTitle = getArguments().getString(ARG_TITLE);
            ...
            if (contexForm == 1) {
                userContract = getArguments().getParcelable(ARG_PARAM4);
                processSuccesRequest = 1;
                idUser = userContract.UserId;
            } else {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (contexForm == 1) {
            ...
            if (userContract.StartDate != null && userContract.FinishDate != null) {
                try {
                    @SuppressLint("SimpleDateFormat")
                    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
                    Date date1 = format.parse(userContract.StartDate);
                    Date date2 = format.parse(userContract.FinishDate);
                    @SuppressLint("SimpleDateFormat")
            ...
            userAutoCompleteTextView.setText(userContract.Email);
        ...
        if (!idContract.equals("")) {
            ...
            String url = Constantes.LIST_ROLES_URL + idCompany;
        ...
        if (roles != null) {
            ...
            if (!cancel) {
                ...
                CrudIntentService.startRequestCRUD(getActivity(),
                        Constantes.LIST_CONTRACTS_URL + idContract + "/Users/", Request.Method.POST, json, "", false);
            }
        }
    }
    ...
    private void Roles(String rolesResult) {
        ...
        if (contexForm == 1) {
            for (int i = 0; i < rolInfoList.size(); i++) {
                if (userContract.RoleId.equals(rolInfoList.get(i).Id)) {
        ...
        if (rolInfoList == null || rolInfoList.isEmpty()) {
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext())
                    ...
                    .setMessage("No se encontraron roles en el contrato");
            builder.create().show();
        }
    }
    ...
    public void onStringResult(String action, int option, String jsonObjStr, String url) {
        ...
        switch (option) {
            ...
            case Constantes.BAD_REQUEST:
                logW("response " + jsonObjStr);
                try {
                    JSONObject error = new JSONObject(jsonObjStr);
                    String message = error.getString("Error");
                    MetodosPublicos.alertDialog(instance, message);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            ...
            case Constantes.REQUEST_NOT_FOUND:
                AlertDialog.Builder builder = new AlertDialog.Builder(getContext())
                        ...
                        .setCancelable(false)
                        .setPositiveButton("Si", (dialogInterface, i) -> userAutoCompleteTextView.setText(""))
