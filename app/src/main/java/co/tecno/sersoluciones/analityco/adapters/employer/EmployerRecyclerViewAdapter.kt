package co.tecno.sersoluciones.analityco.adapters.employer
...
class EmployerRecyclerViewAdapter(
        private val mContext: Context,
        private val mListener: OnListEmployerInteractionListener?,
        private val mValues: ArrayList<ObjectList>
) : RecyclerView.Adapter<EmployerRecyclerViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (holder.mItem != null) {
            if(holder.mItem!!.IsManaged == 1){
                holder.companyManage!!.visibility = View.VISIBLE
            }else {
                holder.companyManage!!.visibility = View.GONE
            }
            holder.textName!!.text = holder.mItem!!.Name
            if(holder.textAddress!!.text.isNullOrEmpty()){
                holder.textAddress!!.visibility = View.GONE
            }
            holder.textAddress!!.text = holder.mItem!!.DocumentType + " " + holder.mItem!!.DocumentNumber
            holder.textValidity!!.text = holder.mItem!!.Address + " " + holder.mItem!!.CityName
            holder.stateIcon!!.visibility = View.VISIBLE
            /* if (holder.mItem!!.IsActive) {
                 if (holder.mItem!!.Expiry) holder.stateIcon!!.setImageResource(R.drawable.state_icon) else holder.stateIcon!!.visibility = View.GONE
             } else {
...
//                    Uri uri = Uri.parse(url);
    ...
    inner class ViewHolder internal constructor(view: View) : RecyclerView.ViewHolder(view) {
        ...
        var companyManage : ImageView? = null
