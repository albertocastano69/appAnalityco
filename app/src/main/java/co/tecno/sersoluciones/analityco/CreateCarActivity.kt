...
import co.tecno.sersoluciones.analityco.databinding.ActivityCreateCarBinding
...
class CreateCarActivity : BaseActivity(), AdapterView.OnItemSelectedListener, RequestBroadcastReceiver.BroadcastListener, TextWatcherAdapter.TextWatcherListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(
                this, R.layout.activity_create_car
        )
            ...
            projectList?.let {
                ...
                for (project in viewModel.projects.value!!) {
                    if (projectList.Name == project.Name) {
                        DebugLog.logW("project found ${projectList.Name} position $i")
    ...
    private fun alertClose() {
        val builder = MaterialAlertDialogBuilder(this)
                .setTitle("Atencion")
                .setMessage("¿Desea salir sin crear este vehiculo?")
                .setPositiveButton("Aceptar") { _, _ ->
                    //viewModel.clearForm()
                    //personalViewModel.clearData()
                    finish()
